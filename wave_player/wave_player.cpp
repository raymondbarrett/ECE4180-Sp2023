...
//-----------------------------------------------------------------------------
void
wave_player::set_verbosity(int v)
{
  verbosity = v;
}
...
//-----------------------------------------------------------------------------
void
wave_player::play(FILE* wavefile)
{
    ...
    switch (chunk_id) {
      ...
      case 0x61746164:
        ...
        if (verbosity) {
          ...
          printf("  %d slices\n", num_slices);
          printf(
            "  Ideal sample interval=%d\n",
            (unsigned)(1000000.0 / wav_format.sample_rate));
          printf("  programmed interrupt tick interval=%d\n", samp_int);
        }

        // starting up ticker to write samples out -- no printfs until
        // tick.detach is called
        if (verbosity)
        ...
        // start reading slices, which contain one sample each for however many
        // channels are in the wave file.  one channel=mono, two
        // channels=stereo, etc.  Since mbed only has a single AnalogOut, all of
        // the channels present are averaged to produce a single sample value.
        // This summing and averaging happens in a variable of type signed long
        // long, to make sure that the data doesn't overflow regardless of
        // sample size (8 bits, 16 bits, 32 bits).
        //
        // note that from what I can find that 8 bit wave files use unsigned
        // data, while 16 and 32 bit wave files use signed data
        //
        for (slice = 0; slice < num_slices; slice += 1) {
          ...
          for (channel = 0; channel < wav_format.num_channels; channel++) {
            switch (wav_format.sig_bps) {
              case 16:
                if (verbosity)
                  printf(
                    "16 bit channel %d data=%d ", channel, data_sptr[channel]);
                slice_value += data_sptr[channel];
                break;
              case 32:
                if (verbosity)
                  printf(
                    "32 bit channel %d data=%d ", channel, data_wptr[channel]);
                slice_value += data_wptr[channel];
                break;
              case 8:
                if (verbosity)
                  printf(
                    "8 bit channel %d data=%d ",
                    channel,
                    (int)data_bptr[channel]);
          ...
          // slice_value is now averaged.  Next it needs to be scaled to an
          // unsigned 16 bit value with DC offset so it can be written to the
          // DAC.
          switch (wav_format.sig_bps) {
            case 8:
              slice_value <<= 8;
          ...
          if (verbosity)
            printf(
              "sample %d wptr %d slice_value %d dac_data %u\n",
              slice,
              DAC_wptr,
              (int)slice_value,
              dac_data);
