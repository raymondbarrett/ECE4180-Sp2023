...
// use the Audio Out jack for connection to a set of amplified PC speakers (at
// higher volume) Needs a USB flash drive inserted with the *.wav files on it to
// run with audio effects Pins are setup for mbed LPC1768 on mbed application
// board Analog Audio Out Jack for PC Speakers- Sparkfun also has a breakout For
// more info see:
...
void
incandescent_bulb(PwmOut led, float on_time = 0.1, float warm_time = 0.2)
{
  // LED warm up effect using PWM
  for (int i = 0; i < 10; i++) {
    led = i / 10.0;
...
main()
{
  ...
  while (1) {
    ...
    } // code block forces thread out of scope reclaims mem-can recreate thread
      // object
    {
