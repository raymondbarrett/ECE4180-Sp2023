...
void
LSM9DS1::init(interface_mode interface, uint8_t xgAddr, uint8_t mAddr)
{
...
LSM9DS1::begin()
{
  ...
  else if (settings.device.commInterface == IMU_MODE_SPI) // else, if we're
                                                          // using SPI
...
// and gyro data, average them, scales them to  gs and deg/s, respectively, and
// then passes the biases to the main sketch for subtraction from all subsequent
// data. There are no gyro and accelerometer bias registers to store the data as
// there are in the ADXL345, a precursor to the LSM9DS0, or the MPU-9150, so we
// have to subtract the biases ourselves. This results in a more accurate
// measurement in general and can remove errors due to imprecise or varying
// initial placement. Calibration of sensor data in this manner is good
// practice.
void
LSM9DS1::calibrate(bool autoCalc)
{
...
void
LSM9DS1::calibrateMag(bool loadIn)
{
  int     i, j;
...
void
LSM9DS1::magOffset(uint8_t axis, int16_t offset)
{
  if (axis > 2)
    return;
  uint8_t msb, lsb;
...
uint8_t
LSM9DS1::magAvailable(lsm9ds1_axis axis)
{
  uint8_t status;
...
int16_t
LSM9DS1::readAccel(lsm9ds1_axis axis)
{
  uint8_t temp[2];
  int16_t value;
...
int16_t
LSM9DS1::readMag(lsm9ds1_axis axis)
{
  uint8_t temp[2];
...
int16_t
LSM9DS1::readGyro(lsm9ds1_axis axis)
{
  uint8_t temp[2];
  int16_t value;
...
float
LSM9DS1::calcAccel(int16_t accel)
{
...
void
LSM9DS1::setGyroScale(uint16_t gScl)
{
...
void
LSM9DS1::setAccelScale(uint8_t aScl)
{
...
void
LSM9DS1::setMagScale(uint8_t mScl)
{
...
void
LSM9DS1::setGyroODR(uint8_t gRate)
{
...
void
LSM9DS1::setAccelODR(uint8_t aRate)
{
...
void
LSM9DS1::setMagODR(uint8_t mRate)
{
...
void
LSM9DS1::configInactivity(uint8_t duration, uint8_t threshold, bool sleepOn)
{
  uint8_t temp = 0;
...
void
LSM9DS1::configAccelInt(uint8_t generator, bool andInterrupts)
{
...
void
LSM9DS1::configGyroInt(uint8_t generator, bool aoi, bool latch)
{
  // Use variables from accel_interrupt_generator, OR'd together to create
  // the [generator]value.
  uint8_t temp = generator;
...
void
LSM9DS1::configMagInt(uint8_t generator, h_lactive activeLow, bool latch)
{
...
void
LSM9DS1::configMagThs(uint16_t threshold)
{
...
void
LSM9DS1::enableFIFO(bool enable)
{
  uint8_t temp = xgReadByte(CTRL_REG9);
  if (enable)
    temp |= (1 << 1);
...
void
LSM9DS1::setFIFO(fifoMode_type fifoMode, uint8_t fifoThs)
{
...
void
LSM9DS1::constrainScales()
{
  if (
    (settings.gyro.scale != 245) && (settings.gyro.scale != 500) &&
    (settings.gyro.scale != 2000)) {
    settings.gyro.scale = 245;
  }

  if (
    (settings.accel.scale != 2) && (settings.accel.scale != 4) &&
    (settings.accel.scale != 8) && (settings.accel.scale != 16)) {
    settings.accel.scale = 2;
  }

  if (
    (settings.mag.scale != 4) && (settings.mag.scale != 8) &&
...
void
LSM9DS1::xgWriteByte(uint8_t subAddress, uint8_t data)
{
...
void
LSM9DS1::mWriteByte(uint8_t subAddress, uint8_t data)
{
  // Whether we're using I2C or SPI, write a byte using the
...
uint8_t
LSM9DS1::xgReadByte(uint8_t subAddress)
{
...
void
LSM9DS1::xgReadBytes(uint8_t subAddress, uint8_t* dest, uint8_t count)
{
...
uint8_t
LSM9DS1::mReadByte(uint8_t subAddress)
{
  // Whether we're using I2C or SPI, read a byte using the
  // accelerometer-specific I2C address or SPI CS pin.
  if (settings.device.commInterface == IMU_MODE_I2C)
...
void
LSM9DS1::mReadBytes(uint8_t subAddress, uint8_t* dest, uint8_t count)
{
  // Whether we're using I2C or SPI, read multiple bytes using the
  // accelerometer-specific I2C address or SPI CS pin.
  if (settings.device.commInterface == IMU_MODE_I2C)
...
void
LSM9DS1::SPIwriteByte(uint8_t csPin, uint8_t subAddress, uint8_t data)
{
  /*
...
uint8_t
LSM9DS1::SPIreadByte(uint8_t csPin, uint8_t subAddress)
{
  uint8_t temp;
...
void
LSM9DS1::I2CwriteByte(uint8_t address, uint8_t subAddress, uint8_t data)
{
  /*
...
uint8_t
LSM9DS1::I2CreadByte(uint8_t address, uint8_t subAddress)
{
  /*
  ...
  buffer Wire.endTransmission(true);             // Send the Tx buffer, but send
  a restart to keep connection alive Wire.requestFrom(address, (uint8_t) 1);  //
  Read one byte from slave register address while ((Wire.available() < 1) &&
  (timeout-- > 0)) delay(1);
  ...
  return data;                             // Return data read from slave
  register
  */
  char data;
  ...
  uint8_t  count)
{
  ...
  Wire.endTransmission(true);             // Send the Tx buffer, but send a
  ...
  count) && (timeout-- > 0)) delay(1); if (timeout <= 0) return -1;
