...
volatile USB_INT32U HOST_WdhIntr =
  0; /* Semaphore to wait until the TD is submitted            */
...
USB_INT16U*
  TDBufNonVol; /* Identical to TDBuffer just to reduce compiler warnings */
volatile USB_INT08U*
  TDBuffer; /* Current Buffer Pointer of transfer descriptor          */
...
void
Host_DelayUS(USB_INT32U delay)
{
  volatile USB_INT32U i;

  for (i = 0; i < (4 * delay);
       i++) { /* This logic was tested. It gives app. 1 micro sec delay */
    ;
  }
}
...
Host_Init(void)
{
  ...
  // 2: reserved
  // 3: U1=host, U2=device
  // NB: this register is only available if OTG clock (aka "port select") is
  // enabled!! since we don't care about port 2, set just bit 0 to 1 (U1=host)
  LPC_USB->OTGStCtrl |= 1;
  ...
  */
  ...
  LPC_USB->HcControl =
    (LPC_USB->HcControl & (~OR_CONTROL_HCFS)) | OR_CONTROL_HC_OPER;
  LPC_USB->HcRhStatus = OR_RH_STATUS_LPSC; /* Set Global Power */

  LPC_USB->HcHCCA = (USB_INT32U)Hcca;
  LPC_USB->HcInterruptStatus |=
    LPC_USB->HcInterruptStatus; /* Clear Interrrupt Status                    */

  LPC_USB->HcInterruptEnable =
    OR_INTR_ENABLE_MIE | OR_INTR_ENABLE_WDH | OR_INTR_ENABLE_RHSC;
...
USB_IRQHandler(void) __irq
{
  ...
  if (!(int_status & ie_status)) {
      ...
      if (LPC_USB->HcRhPortStatus1 & OR_RH_PORT_CSC) {
        if (LPC_USB->HcRhStatus & OR_RH_STATUS_DRWE) {
          ...
          if (LPC_USB->HcRhPortStatus1 & OR_RH_PORT_CCS) {
            ...
            if (gUSBConnected) {
              LPC_USB->HcInterruptEnable =
                0; // why do we get multiple disc. rupts???
              HOST_RhscIntr = 0;
              gUSBConnected = 0;
            } else
    ...
    LPC_USB->HcInterruptStatus =
      int_status; /* Clear interrupt status register      */
  }
  return;
}
 ...
 *
 * Arguments  : ed            Endpoint descriptor that contains this transfer
 *descriptor token         SETUP, IN, OUT buffer        Current Buffer Pointer
 *of the transfer descriptor buffer_len    Length of the buffer
 *
  ...
  USB_INT32U           buffer_len)
{
  ...
  TDHead->Control = (TD_ROUNDING | token | TD_DELAY_INT(0) | td_toggle | TD_CC);
  TDTail->Control = 0;
...
Host_EnumDev(void)
{
  ...
  Host_DelayMS(2);
  EDCtrl->Control =
    (EDCtrl->Control) | 1; /* Modify control pipe with address 1 */
  ...
  rc = HOST_GET_DESCRIPTOR(
    USB_DESCRIPTOR_TYPE_CONFIGURATION, 0, TDBuffer, ReadLE16U(&TDBuffer[2]));
  if (rc != OK) {
    PRINT_Err(rc);
    return (rc);
  }
...
/*
 *                                 INITIALIZE HOST CONTROLLER COMMUNICATIONS
 * AREA
 *
...
void
Host_HCCAInit(volatile HCCA* hcca)
{
  USB_INT32U i;
 ...
 *
 * Description: This function is infinite loop which breaks when ever a WDH
 *interrupt rises
 *
 * Arguments  : None
 *
 ...
 *                                         READ LE 32U
 *
 * Description: This function is used to read an unsigned integer from a
 *character buffer in the platform containing little endian processor
 *
...
USB_INT32U
ReadLE32U(volatile USB_INT08U* pmem)
{
 ...
 *                                        WRITE LE 32U
 *
 * Description: This function is used to write an unsigned integer into a
 *charecter buffer in the platform containing little endian processor.
 *
...
void
WriteLE32U(volatile USB_INT08U* pmem, USB_INT32U val)
{
#ifdef __BIG_ENDIAN
 ...
 *                                          READ LE 16U
 *
 * Description: This function is used to read an unsigned short integer from a
 *charecter buffer in the platform containing little endian processor
 *
 * Arguments  : pmem    Pointer to the charecter buffer
 *
...
USB_INT16U
ReadLE16U(volatile USB_INT08U* pmem)
{
 ...
 *                                         WRITE LE 16U
 *
 * Description: This function is used to write an unsigned short integer into a
 *charecter buffer in the platform containing little endian processor
 *
 * Arguments  : pmem    Pointer to the charecter buffer
...
void
WriteLE16U(volatile USB_INT08U* pmem, USB_INT16U val)
{
#ifdef __BIG_ENDIAN
 ...
 *
 * Description: This function is used to read an unsigned integer from a
 *charecter buffer in the platform containing big endian processor
 *
 * Arguments  : pmem    Pointer to the charecter buffer
 *
 * Returns    : val     Unsigned integer
...
USB_INT32U
ReadBE32U(volatile USB_INT08U* pmem)
{
  USB_INT32U val = *(USB_INT32U*)pmem;
#ifdef __BIG_ENDIAN
  return val;
...
void
WriteBE32U(volatile USB_INT08U* pmem, USB_INT32U val)
{
#ifdef __BIG_ENDIAN
  *(USB_INT32U*)pmem = val;
#else
...
USB_INT16U
ReadBE16U(volatile USB_INT08U* pmem)
{
  USB_INT16U val = *(USB_INT16U*)pmem;
#ifdef __BIG_ENDIAN
  return val;
 ...
 *                                         WRITE BE 16U
 *
 * Description: This function is used to write an unsigned short integer into
 *the charecter buffer in the platform containing big endian processor
 *
 * Arguments  : pmem    Pointer to the charecter buffer
 *              val     Value to be placed in the charecter buffer
 *
...
void
WriteBE16U(volatile USB_INT08U* pmem, USB_INT16U val)
{
#ifdef __BIG_ENDIAN
  *(USB_INT16U*)pmem = val;
#else
