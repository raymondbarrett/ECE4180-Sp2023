...
USB_INT32S
MS_Init(USB_INT32U* blkSize, USB_INT32U* numBlks, USB_INT08U* inquiryResult)
{
  USB_INT08U retry;
  USB_INT32S rc;
  ...
  while (desc_ptr != TDBuffer + ReadLE16U(&TDBuffer[2])) {
    ...
    switch (desc_ptr[1]) {
      ...
      case USB_DESCRIPTOR_TYPE_INTERFACE: /* If it is an interface descriptor */
        if (
          desc_ptr[5] ==
            MASS_STORAGE_CLASS && /* check if the class is mass storage */
          desc_ptr[6] ==
            MASS_STORAGE_SUBCLASS_SCSI && /* check if the subclass is SCSI */
          desc_ptr[7] ==
            MASS_STORAGE_PROTOCOL_BO) { /* check if the protocol is Bulk only */
          ms_int_found = 1;
 ...
 *                                            READ CAPACITY
 *
 * Description: This function is used to read the capacity of the mass storage
 *device
 *
 * Arguments  : None
 *
...
USB_INT32S
MS_ReadCapacity(USB_INT32U* numBlks, USB_INT32U* blkSize)
{
  USB_INT32S rc;
...
USB_INT32S
MS_Inquire(USB_INT08U* response)
{
  USB_INT32S rc;
  USB_INT32U i;
  ...
  volatile USB_INT08U* user_buffer)
{
  ...
  if (rc == OK) {
    rc =
      Host_ProcessTD(EDBulkOut, TD_OUT, user_buffer, MS_BlkSize * num_blocks);
    if (rc == OK) {
      rc = Host_ProcessTD(EDBulkIn, TD_IN, TDBuffer, CSW_SIZE);
      if (rc == OK) {
        if (TDBuffer[12] != 0) {
